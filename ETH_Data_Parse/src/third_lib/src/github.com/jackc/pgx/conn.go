	...
	for {
		...
		case *pgproto3.ReadyForQuery:
			...
			if c.ConnInfo == minimalConnInfo {
				var lock_timeout string
				rows, err := c.Query("show lock_timeout")
				if err != nil {
					return err
				}

				if rows.Next() {
					err = rows.Scan(&lock_timeout)
					if err != nil {
						return err
					}
				}
				...
				rows, err = c.Query("set lock_timeout TO 15000")
				if err != nil {
					return err
				}
				rows.Close()
				...
				err = c.initConnInfo()
				if err != nil {
					rows, err2 := c.Query(fmt.Sprintf("set lock_timeout TO %s", lock_timeout))
					if err2 != nil {
